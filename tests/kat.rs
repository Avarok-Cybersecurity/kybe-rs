...
fn kat_kyber512() {
    ...
    for test in kat.tests {
        ...
        let k_recovered = kem.decaps(&ct, &sk).unwrap();
...
fn kat_kyber1024() {
    ...
    for test in kat.tests {
        ...
        let ss = ByteArray::from_bytes(decode(test.ss).expect("failed to decode").as_slice());
        let ct = ByteArray::from_bytes(decode(test.ct).expect("failed to decode").as_slice());
        let seed = ByteArray::from_bytes(decode(test.seed).expect("failed to decode").as_slice());

        let (c, k) = kem.encaps_with_seed(&pk, seed).unwrap();
        let k_recovered = kem.decaps(&c, &sk).unwrap();
